...
class PHPSDKTestCase extends PHPUnit_Framework_TestCase
{
  ...
  public function testGraphAPIMethodOAuthSpecError() {
    ...
    try {
      ...
      // ProfileDelete means the server understood the DELETE
      $msg = 'invalid_request: (#2903) Cannot delete this test account because it is associated with other applications. Use DELETE app_id/accounts/test-users/test_account_id to remove it from other apps first. Use GET test_user_id/ownerapps to get complete list of owner apps.';
      $this->assertEquals($msg, (string) $e,
                          'Expect the invalid session message.');
    }
  }
  ...
  public function testBundledCACert() {
    ...
    Facebook::$CURL_OPTS[CURLOPT_CAINFO] = ROOT_BACK . 'includes/facebook/fb_ca_chain_bundle.crt';
    $response = $facebook->api('/naitik');
